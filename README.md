...
Runtime dependency DLLs are provided by Conan and are **not bundled in this repository**.  
Before launching the renderer, activate the Conan-generated runtime environment.

The renderer also expects its copied shaders, models, textures, and fonts relative to the build output directory, so run the executable with that directory as the working directory.
...
cd /d build\Release\bin
OpenGL_Course_Project.exe
```

#### PowerShell

A `.bat` file cannot modify the environment of its parent PowerShell process, so run the environment setup and executable in the same `cmd` process:

```powershell
cmd /c "call build\Release\generators\conanrun.bat && cd /d build\Release\bin && OpenGL_Course_Project.exe"
```
...
```bat
call build\Debug\generators\conanrun.bat
cd /d build\Debug\bin
OpenGL_Course_Projectd.exe
```

or from PowerShell:

```powershell
cmd /c "call build\Debug\generators\conanrun.bat && cd /d build\Debug\bin && OpenGL_Course_Projectd.exe"
```

The `conanrun.bat` files are generated by `conan install` and configure the runtime search paths for Conan-managed shared libraries such as GLFW and Assimp.

CMake copies the runtime assets into the build output directory as part of the build.
...
This project contains tutorial-derived code, bundled libraries, fonts, and external assets that remain under their respective licenses.

See [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) for the current provenance and attribution record, and [`ASSET_LICENSE_AUDIT.md`](ASSET_LICENSE_AUDIT.md) for the repository cleanup/audit notes.

The DDS loader's historical upstream repository has been located under its current GitHub name and is MIT-licensed. Poly Haven assets used by the project are CC0. The immediate source of the modified Sponza model is documented, but the exact redistribution license of that raw asset bundle is intentionally not asserted.
