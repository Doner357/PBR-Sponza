...
vec3 CalcShadowDirLight(DirLight light, vec3 N, vec3 V, vec3 F0, int shadow_id) {
	...
	vec4 light_space_frag_pos = dirLightSpaceMat[shadow_id] * vec4(fs_in.fragPos, 1.0);
	float shadow = DirLightShadowCalculation(light, light_space_frag_pos, N, L, shadow_id);
...
vec3 CalcShadowPointLight(PointLight light, vec3 N, vec3 V, vec3 F0, int shadow_id) {
	...
	float shadow = PointLightShadowCalculation(light, fs_in.fragPos, N, shadow_id);
...
vec3 CalcShadowSpotLight(SpotLight light, vec3 N, vec3 V, vec3 F0, int shadow_id) {
	...
	vec4 light_space_frag_pos = spotLightSpaceMat[shadow_id] * vec4(fs_in.fragPos, 1.0);
	float shadow = SpotLightShadowCalculation(light, light_space_frag_pos, N, L, shadow_id);
